...
const ProjectsPage: React.FC = () => {
  ...
  const renderProjectCard = (
    ...
    <Card
      key={p.id}
      className="bg-white/10 backdrop-blur rounded-lg p-4 hover:shadow-xl transition duration-300"
    >
          ...
          {p.images.map((img, idx) => (
            <CarouselItem key={idx}>
              <img
                src={img}
                alt={`${p.title} ${idx + 1}`}
                className="w-full h-52 md:h-64 object-cover rounded-md mb-3 cursor-pointer"
                onClick={() => openModal(img, `${p.title} ${idx + 1}`)}
              />
            </CarouselItem>
          ))}
        </CarouselContent>
        <CarouselPrevious className="absolute left-2 top-1/2 -translate-y-1/2 rounded-full bg-black bg-opacity-50 p-2 text-white hover:bg-opacity-75 transition">
          ‹
        </CarouselPrevious>
        <CarouselNext className="absolute right-2 top-1/2 -translate-y-1/2 rounded-full bg-black bg-opacity-50 p-2 text-white hover:bg-opacity-75 transition">
          ›
        </CarouselNext>
      </Carousel>
  ...
  return (
    <AnimatedSection className="py-20 px-6 text-white">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
        <div>
          <h3 className="text-2xl font-semibold mb-6">Projelerimiz</h3>
          <div className="space-y-6">{ourProjects.map(renderProjectCard)}</div>
        </div>
        <div>
          <h3 className="text-2xl font-semibold mb-6">Ortak Projelerimiz</h3>
